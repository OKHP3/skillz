---
name: okhp3-sharepoint-list-schema-design
description: >
...
metadata:
  ...
  version: "1.1.0"
  category: copilot
  target-host: sharepoint
  host-support: not-verified
  origin: okhp3/skillz
...
# okhp3-sharepoint-list-schema-design
...
This is a portable, draft-producing SharePoint list-design contract. The
`copilot` family is its catalog home; native Copilot in SharePoint discovery,
Agent Assets packaging, and execution have not been verified. Supply the schema,
build cards, runbook, and applicable decisions to a host that can read them.
If those inputs are unavailable, state the gap before proposing a design.
The skill does not provision lists or change a tenant. Any later implementation
requires a separately authorized workflow and the site's existing permissions.
