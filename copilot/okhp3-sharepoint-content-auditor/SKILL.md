---
name: okhp3-sharepoint-content-auditor
description: >
...
# okhp3-sharepoint-content-auditor
...
This is a portable, read-only SharePoint screenshot-review contract. The
`copilot` family is its catalog home; native Copilot in SharePoint discovery,
Agent Assets packaging, image inspection, and execution have not been verified.
Use a host that can inspect the supplied screenshots and reference documents.
If either input is inaccessible, report the missing evidence and request it
instead of inferring settings. No connector or live tenant access is required
or authorized. The rename preserves the existing list-build audit scope.
