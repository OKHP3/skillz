  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: 'Maturity and evidence are two separate questions, and Skillz Forge never merges them. Maturity describes the state of the contract itself: Placeholder (directory exists, no content), Skeleton (basic contract shape, important behavior incomplete), Draftable (an agent can follow the workflow under supervision — this does not mean validated), Usable (a defined workflow has been exercised and its limits documented — a track-record claim about the contract, not a claim that an eval or benchmark record exists), Validated (at least one recorded eval or benchmark artifact backs the contract — check the evidence note for whether it is current, historical, or design-only), Published (stable public release backed by a live, version-matched evaluation). Evidence describes what proof exists for the current version: none, local checks, designed, analytical, not-run, historical, or live. A skill can be Draftable with live evidence, or Usable with no evidence record at all — the evidence note on each skill page tells you which, and never contradicts the maturity label. Published specifically requires fresh, version-matched evaluation evidence; a historical or not-run note on a lower maturity level means that evidence does not yet exist for the current version. Lower-maturity and lower-evidence skills are shown clearly — they are not hidden.',
        links: [
      ...
      },
      {
        ...
        answer: 'Check both the maturity label and the evidence note on the skill\'s detail page. Published requires a fresh, version-matched benchmark with a measurable quality gap. Validated requires only that at least one recorded eval case or benchmark run exists — it can be historical or analytical rather than a current, live-graded run; check the evidence note for which. Usable means a workflow has been exercised with documented limits, but it is not a production-safety guarantee for every environment and does not require any recorded eval or benchmark artifact. Skeleton and Draftable mean the contract is functional but not yet benchmarked — an evidence note of "historical" means an older version was benchmarked, not the current one; "not-run" means the evaluation design exists but has not been executed; "none" means no indexed evidence record exists, which is not the same as unsafe. Placeholder means the directory is a reserved spot, nothing more.',
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: 'Production-quality skills are validated through the okhp3-skill-foundry 8-phase methodology: architecture, draft, eval design, live execution, grading, benchmark, fix loop, and description optimization. The primary quality signal is the with/without gap — a skill that scores 1.0 with skill access and 0.3 without is doing real work. Reaching Validated requires at least one recorded eval case or benchmark run to exist for the skill — a bare test file or benchmark design with no graded run does not qualify. That artifact does not have to match the skill\'s current version to earn Validated; a benchmark run against an earlier version is recorded as "historical" evidence rather than "live." Requiring a current-version, live-graded run is the bar for Published, not Validated — check the evidence note on each skill\'s detail page for which one it has.',
        links: [
