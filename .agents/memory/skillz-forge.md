...
description: React+Vite SPA for the OKHP3/skillz repository — Phase 1 complete, lives in artifacts/forge/
---

## What was built

Skillz Forge is the public discovery, search, and installation SPA at `artifacts/forge/` for the OKHP3/skillz repository.

## Key architecture facts

- **Catalog generation**: `artifacts/forge/scripts/build-catalog.js` — walks repo root, finds all SKILL.md files at depth ≤ 3, parses YAML frontmatter + body, outputs `artifacts/forge/public/data/catalog.json`. Run with `node artifacts/forge/scripts/build-catalog.js` from repo root. Per-skill `lastModified` and `commitSha` are extracted via `git log -n 1 --format="%aI %H"` for each file. See "displayName derivation" section below.
- **Framework**: React 19 + Vite 8, TypeScript, React Router v7. No component library — fully custom CSS from design tokens.
- **Search**: Fuse.js client-side index (`artifacts/forge/src/utils/search.ts`). Indexes: name, displayName, description, triggers, inputs, outputs, family, category, topics, tools, runtimes, companions, boundaries, examples. Search runs entirely in-browser with URL-persisted filters (q, family, maturity, sort).
- **Port**: Runs on port 5000 through the managed Forge workflow.
- **Build**: `pnpm --filter @workspace/forge run build` — production build to `artifacts/forge/dist/public/`.

## Design system

Tokens in `artifacts/forge/src/index.css`. Canonical OverKill Hill P³™ palette: espresso `#2a2320` (bg), teal `#1c3a34` (structural), orange `#c46a2c` (action/copper), amber `#e6a03c` (highlight), paper `#f6f2ee` (inspection panels). Typography: **Alfa Slab One** (display headings) + **DM Sans** (body/UI) + **JetBrains Mono** (code/paths). Do NOT use Playfair Display or Inter — those are stale references.
...
## Why: adding React locally to artifacts/forge/

React and react-dom must be in `artifacts/forge/package.json` directly — if installed globally via installLanguagePackages(), pnpm hoisting creates multiple React copies and throws invalid hook call. Always install into the Forge artifact explicitly.

## Stacks and FAQ

Static data in `artifacts/forge/src/data/stacks.ts` (5 curated stacks) and `artifacts/forge/src/data/faq.ts`. Edit these files to add/update stacks and FAQ content.

## GitHub URLs

The catalog uses `OKHP3/skillz` as the repo slug. If the repo is renamed, update `GITHUB_BASE` and `RAW_BASE` in `artifacts/forge/scripts/build-catalog.js` and `repoUrl()` in `artifacts/forge/src/utils/github.ts`.
...
The `Family` interface in `artifacts/forge/src/types/catalog.ts` includes `displayName: string`.
