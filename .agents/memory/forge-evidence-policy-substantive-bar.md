...
`applyEvidencePolicy()` in `artifacts/forge/scripts/build-catalog.js` downgrades an unsupported "validated" claim using `hasSubstantiveEvidenceArtifact()` (requires an eval or benchmark artifact), not the looser `hasAnyEvidenceArtifact()` (which also counts a bare test/script file and still gates the lower "usable" bar).
