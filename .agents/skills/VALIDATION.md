...
| `archive-link-validation` | After moving or editing preserved migration documents | Confirms the archived README notice and ledger scope remain present, and every local Markdown/image destination resolves from the archive location | Non-zero exit for missing targets, root-escaping paths, or missing archive-scope documentation |
