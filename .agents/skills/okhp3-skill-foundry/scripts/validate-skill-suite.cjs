...
const COPILOT_PACKAGE_NAME_MAX = 50;
...
function validateSkill(skillDir, rootDir) {
  ...
  const isCopilotPackage = path.basename(path.dirname(skillDir)) === 'copilot';
  const packageNameMax = isCopilotPackage ? COPILOT_PACKAGE_NAME_MAX : PACKAGE_NAME_MAX;
  ...
  if (entry.name.length > packageNameMax) fail(`${entry.name}: package directory exceeds the ${packageNameMax}-character ${isCopilotPackage ? 'Copilot family' : 'portable repository'} limit`);
