...
**Why:** discovered when adding a "skip to pagination" in-page link on Explore and re-testing the pre-existing global "Skip to main content" link in `forge/src/App.tsx` — both were real anchors and both broke navigation when activated, even though this had gone unnoticed before (nobody keyboard-tested them).
