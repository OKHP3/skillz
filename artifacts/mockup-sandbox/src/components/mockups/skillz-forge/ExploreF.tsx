...
type ShelfKey = "all" | "universal" | "process-capture" | "knowledge-operations" | "context-extraction" | "agent-foundry" | "community";
...
const shelves: { key: ShelfKey; label: string; count: string; color: string }[] = [
  { key: "all", label: "Complete index", count: "146", color: "#bf6044" },
  { key: "universal", label: "Universal tools", count: "18", color: "#b98a46" },
  { key: "process-capture", label: "Process capture", count: "09", color: "#4b8275" },
  { key: "knowledge-operations", label: "Knowledge ops", count: "11", color: "#668aa0" },
  { key: "context-extraction", label: "Context extraction", count: "08", color: "#927844" },
  { key: "agent-foundry", label: "Agent foundry", count: "14", color: "#ab6252" },
  { key: "community", label: "Community", count: "10", color: "#5b8c7a" },
];
...
const skills: Skill[] = [
  {
    ...
    note: "The first pair of eyes on a new contract.",
    sections: [
      { title: "Why it exists", body: "When a registry grows faster than its memory, useful work becomes difficult to find. The cataloger makes the collection visible without flattening the nuance inside each contract." },
      { title: "What it returns", body: "A normalized index with family, maturity, owner, version, entry conditions, and a signal-rich description. The original contract stays intact." },
      { title: "Use it when", body: "You are onboarding a new body of skills, auditing drift, or preparing a handoff where the next person needs the map before the territory." },
    ],
  },
  {
    ...
    note: "The small hinge between thought and work.",
    sections: [
  ...
  },
  {
    ...
    readTime: "5 min",
    tags: ["notes", "process"],
    note: "From the margin to the manual.",
    sections: [
  ...
  },
  {
    ...
    readTime: "7 min",
    tags: ["agents", "design"],
    note: "Architecture for capable things.",
    sections: [
...
const maturityStyle: Record<Maturity, { text: string; background: string; border: string }> = {
  Ready: { text: "#a95337", background: "#f5dfd1", border: "#d69c84" },
  Validated: { text: "#2f6e63", background: "#dcece4", border: "#9bc2b3" },
  Experimental: { text: "#846735", background: "#eee5cf", border: "#cab98a" },
};

const initialCollections: Collection[] = [
  { id: "reading", name: "Reading list", note: "For the quiet hour", color: "#bf6044", skills: ["okhp3-skill-cataloger", "extract-the-context"] },
  { id: "field-kit", name: "Field kit", note: "Reliable on the ground", color: "#4b8275", skills: ["capture-the-work", "fieldnote-to-process"] },
  { id: "foundry", name: "Foundry notes", note: "Before the next build", color: "#668aa0", skills: ["agent-blueprint"] },
];

export function ExploreF() {
  ...
  const [expanded, setExpanded] = useState(["Why it exists"]);
  ...
  const visibleSkills = useMemo(() => {
    ...
    return skills.filter((skill) => {
      ...
      const matchesQuery = !normalized || [skill.name, skill.familyLabel, skill.description, skill.note, ...skill.tags].some((value) => value.toLowerCase().includes(normalized));
  ...
  const selectedFamily = shelves.find((shelf) => shelf.key === selected.family);
  const savedIn = collections.filter((collection) => collection.skills.includes(selected.name));
  const savedCount = collections.reduce((total, collection) => total + collection.skills.length, 0);

  function chooseSkill(name: string) {
    setSelectedName(name);
    const next = skills.find((skill) => skill.name === name);
    setExpanded(next ? [next.sections[0].title] : []);
  }
  ...
  function createCollection() {
    ...
    setCollections((current) => [...current, { id, name: cleanName, note: "A new corner of the shelf", color: "#b98a46", skills: [selected.name] }]);
  ...
  return (
    <main className="min-h-[100dvh] w-full overflow-x-hidden" style={{ background: "#eee9df", color: "#293731", fontFamily: "'DM Sans', sans-serif" }}>
      <div className="pointer-events-none fixed inset-0 z-50 opacity-[0.02]" style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 140 140' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='grain'%3E%3CfeTurbulence baseFrequency='.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23grain)'/%3E%3C/svg%3E\")" }} />
      <header className="flex min-h-[68px] items-center justify-between border-b px-5 sm:px-8" style={{ borderColor: "#cfc6b8", background: "#e7e1d6" }}>
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center border" style={{ borderColor: "#bf6044", color: "#a95337" }}><Layers3 size={17} /></div>
          <div className="leading-none"><div className="text-[13px] font-bold tracking-[.18em]">SKILLZ FORGE</div><div className="mt-1 font-mono text-[8px] tracking-[.15em]" style={{ color: "#7f8177" }}>OVERKILL HILL P³ / READER</div></div>
          <div className="hidden h-5 w-px sm:block" style={{ background: "#cfc6b8" }} />
          <span className="hidden font-mono text-[10px] uppercase tracking-[.14em] sm:block" style={{ color: "#777b70" }}>Explore / signal index</span>
        </div>
        <div className="flex items-center gap-4"><div className="hidden items-center gap-2 text-[10px] md:flex" style={{ color: "#4c8176" }}><span className="h-1.5 w-1.5 rounded-full" style={{ background: "#4c8176" }} /> registry synced</div><div className="flex h-8 w-8 items-center justify-center rounded-full font-mono text-[10px]" style={{ background: "#d9c3a7", color: "#80533d" }}>OH</div></div>
      </header>

      <div className="mx-auto max-w-[1540px] px-4 pb-12 sm:px-7">
        <section className="border-b py-8 sm:py-10" style={{ borderColor: "#cfc6b8" }}>
          <div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-end">
            <div>
              <div className="mb-3 flex items-center gap-2 font-mono text-[9px] uppercase tracking-[.22em]" style={{ color: "#a95337" }}><span className="h-px w-8" style={{ background: "#bf6044" }} /> 02 / read the index</div>
              <h1 className="max-w-[760px] text-[clamp(37px,5.3vw,76px)] leading-[.91] tracking-[-.055em]" style={{ fontFamily: "Georgia, serif" }}>Choose by signal,<br /><em style={{ color: "#b2573b", fontStyle: "normal" }}>keep what matters.</em></h1>
            ...
            <div className="flex items-end gap-7 border-l pl-5" style={{ borderColor: "#b8aa99" }}>
              ...
              <div><div className="font-mono text-[28px] leading-none">{savedCount}</div><div className="mt-2 text-[9px] uppercase tracking-[.17em]" style={{ color: "#818077" }}>saved passages</div></div>
            </div>
          </div>
        </section>

        <section className="sticky top-0 z-20 -mx-1 border-b py-4 sm:mx-0" style={{ borderColor: "#cfc6b8", background: "rgba(238,233,223,.97)" }}>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="relative min-w-0 flex-1">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2" size={16} style={{ color: "#b2573b" }} />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, job, or tag…" className="h-11 w-full border bg-[#f7f3eb] pl-11 pr-24 text-[12px] outline-none placeholder:text-[#929188] focus:border-[#b2573b]" style={{ borderColor: query ? "#b2573b" : "#b8aa99", color: "#293731" }} />
          ...
          {showCollections && <div className="mt-4 grid gap-2 border-t pt-4 sm:grid-cols-2 xl:grid-cols-4" style={{ borderColor: "#d8d0c4" }}>
            <button type="button" onClick={() => setActiveCollection("all")} className="flex items-center justify-between border p-3 text-left" style={{ borderColor: activeCollection === "all" ? "#b2573b" : "#cfc6b8", background: activeCollection === "all" ? "#f4ddd0" : "#f4f0e8" }}><span><span className="block text-[11px] font-semibold">All saved</span><span className="mt-1 block font-mono text-[9px]" style={{ color: "#89877d" }}>Every marked passage</span></span><span className="font-mono text-[12px]" style={{ color: "#b2573b" }}>{savedCount}</span></button>
            {collections.map((collection) => <button key={collection.id} type="button" onClick={() => setActiveCollection(collection.id)} className="flex items-center justify-between border p-3 text-left" style={{ borderColor: activeCollection === collection.id ? collection.color : "#cfc6b8", background: activeCollection === collection.id ? "#f5eadc" : "#f4f0e8" }}><span className="min-w-0"><span className="flex items-center gap-2 text-[11px] font-semibold"><span className="h-2 w-2 rounded-full" style={{ background: collection.color }} />{collection.name}</span><span className="mt-1 block truncate font-mono text-[9px]" style={{ color: "#89877d" }}>{collection.note}</span></span><span className="font-mono text-[12px]" style={{ color: collection.color }}>{collection.skills.length}</span></button>)}
          </div>}
          {showFilters && <div className="mt-4 flex flex-wrap items-center gap-2 border-t pt-4" style={{ borderColor: "#d8d0c4" }}><span className="mr-1 font-mono text-[9px] uppercase tracking-[.14em]" style={{ color: "#89877d" }}>maturity</span>{(["all", "Ready", "Validated", "Experimental"] as const).map((maturity) => <button key={maturity} type="button" onClick={() => setActiveMaturity(maturity)} className="border px-2.5 py-1.5 text-[10px]" style={{ borderColor: activeMaturity === maturity ? "#4c8176" : "#cfc6b8", background: activeMaturity === maturity ? "#deebe4" : "transparent", color: activeMaturity === maturity ? "#2f6e63" : "#777b70" }}>{maturity === "all" ? "Any stage" : maturity}</button>)}</div>}
        </section>
        ...
        <div className="grid grid-cols-1 gap-6 py-6 lg:grid-cols-[188px_minmax(0,1fr)_390px] lg:items-start">
          <aside className="hidden lg:block">
            ...
            <nav className="space-y-1">{shelves.map((shelf) => <button key={shelf.key} type="button" onClick={() => { setActiveShelf(shelf.key); setActiveCollection("all"); }} className="group flex w-full items-start justify-between gap-2 border-l-2 px-3 py-2.5 text-left transition-colors" style={{ borderColor: activeShelf === shelf.key ? shelf.color : "transparent", background: activeShelf === shelf.key ? "#e3ddd1" : "transparent", color: activeShelf === shelf.key ? "#293731" : "#777b70" }}><span className="text-[11px] leading-4">{shelf.label}</span><span className="font-mono text-[9px]" style={{ color: activeShelf === shelf.key ? shelf.color : "#aaa69d" }}>{shelf.count}</span></button>)}</nav>
            <div className="mt-7 border-t pt-4" style={{ borderColor: "#cfc6b8" }}><div className="mb-3 font-mono text-[9px] uppercase tracking-[.14em]" style={{ color: "#818077" }}>your shelf</div>{collections.map((collection) => <button key={collection.id} type="button" onClick={() => { setActiveCollection(collection.id); setActiveShelf("all"); }} className="mb-2 flex w-full items-center gap-2 text-left text-[10px]" style={{ color: "#626b63" }}><span className="h-2 w-2 rounded-full" style={{ background: collection.color }} />{collection.name}<span className="ml-auto font-mono text-[9px]" style={{ color: "#aaa69d" }}>{collection.skills.length}</span></button>)}<button type="button" onClick={() => setShowNewCollection(true)} className="mt-2 flex items-center gap-2 text-[10px]" style={{ color: "#a95236" }}><FolderPlus size={12} /> new collection</button></div>
          </aside>

          <section className="min-w-0 border" style={{ borderColor: "#cfc6b8", background: "#f7f3eb" }}>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3.5" style={{ borderColor: "#d8d0c4" }}><div className="flex items-center gap-2"><BookOpen size={14} style={{ color: "#b2573b" }} /><span className="font-mono text-[10px] uppercase tracking-[.16em]" style={{ color: "#646d65" }}>{activeCollection === "all" ? "Live contract set" : collections.find((collection) => collection.id === activeCollection)?.name}</span><span className="font-mono text-[10px]" style={{ color: "#a09c91" }}>· {visibleSkills.length} showing</span></div><span className="font-mono text-[9px] uppercase tracking-[.12em]" style={{ color: "#aaa69d" }}>ordered by signal</span></div>
            ...
            {visibleSkills.map((skill, index) => {
              ...
              const family = shelves.find((shelf) => shelf.key === skill.family);
              return <button key={skill.name} type="button" onClick={() => chooseSkill(skill.name)} className="group grid w-full grid-cols-[minmax(0,1fr)_26px] gap-3 border-b px-4 py-4 text-left transition-colors sm:grid-cols-[minmax(0,1.5fr)_120px_94px_58px_20px] sm:items-center" style={{ borderColor: "#e1dbd0", background: active ? "#f1e1d7" : "transparent", boxShadow: active ? "inset 3px 0 #bf6044" : "none" }}>
                <div className="min-w-0"><div className="flex items-center gap-2"><span className="font-mono text-[11px] font-semibold" style={{ color: active ? "#8f4935" : "#35423c" }}>{skill.name}</span>{index < 2 && <Sparkles size={11} className="shrink-0" style={{ color: "#bd8c42" }} />}</div><div className="mt-1 truncate text-[11px]" style={{ color: "#81877f" }}>{skill.description}</div><div className="mt-2 flex items-center gap-2 sm:hidden"><span className="text-[9px]" style={{ color: family?.color }}>{skill.familyLabel}</span><span className="text-[9px]" style={{ color: status.text }}>· {skill.maturity}</span></div></div>
                <span className="hidden truncate text-[10px] sm:block" style={{ color: family?.color }}>{skill.familyLabel}</span><span className="hidden w-fit border px-1.5 py-1 text-[9px] sm:block" style={{ color: status.text, borderColor: status.border, background: status.background }}>{skill.maturity}</span><span className="hidden items-center gap-1 font-mono text-[10px] sm:flex" style={{ color: "#8e9087" }}><Clock3 size={11} /> {skill.readTime}</span><ArrowUpRight size={14} className="mt-1 transition-colors group-hover:text-[#b2573b]" style={{ color: active ? "#b2573b" : "#a7a49a" }} />
              </button>;
            })}
            {visibleSkills.length === 0 && <div className="flex min-h-[280px] flex-col items-center justify-center px-5 text-center"><Search size={25} style={{ color: "#b2573b" }} /><div className="mt-3 text-[18px]" style={{ fontFamily: "Georgia, serif" }}>Nothing on this shelf.</div><p className="mt-2 text-[11px]" style={{ color: "#85877f" }}>Try another signal, or return to the complete index.</p><button type="button" onClick={resetView} className="mt-4 border px-3 py-2 text-[10px] uppercase tracking-[.12em]" style={{ borderColor: "#b2573b", color: "#a95236" }}>Reset view</button></div>}
          </section>

          <aside className="self-start border lg:sticky lg:top-[90px]" style={{ borderColor: "#b8aa99", background: "#e5ddd1" }}>
            <div className="flex items-center justify-between border-b px-4 py-3.5" style={{ borderColor: "#cfc6b8" }}><div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.15em]" style={{ color: "#59655d" }}><FileText size={13} style={{ color: "#b2573b" }} /> reading pane</div><div className="flex items-center gap-2"><span className="font-mono text-[9px]" style={{ color: "#99958b" }}>open / 01</span><MoreHorizontal size={15} style={{ color: "#99958b" }} /></div></div>
            <div className="p-5">
              <div className="mb-3 flex items-center justify-between gap-3"><div className="font-mono text-[9px] uppercase tracking-[.17em]" style={{ color: selectedFamily?.color }}>{selected.familyLabel} / {selected.note}</div><span className="border px-2 py-1 font-mono text-[9px]" style={{ color: selectedStatus.text, borderColor: selectedStatus.border, background: selectedStatus.background }}>{selected.maturity}</span></div>
              <h2 className="break-words text-[29px] leading-[.99] tracking-[-.035em]" style={{ fontFamily: "Georgia, serif" }}>{selected.name}</h2>
              <p className="mt-4 text-[13px] leading-6" style={{ color: "#646d65" }}>{selected.description}</p>
              <div className="mt-5 flex items-center gap-4 border-y py-3 font-mono text-[9px] uppercase tracking-[.1em]" style={{ borderColor: "#cfc6b8", color: "#85877f" }}><span><strong className="font-normal" style={{ color: "#a95236" }}>{selected.version}</strong> version</span><span><strong className="font-normal">{selected.runs}</strong> runs</span><span><strong className="font-normal">{selected.readTime}</strong> read</span></div>
              ...
              <div className="mt-6 space-y-1.5">{selected.sections.map((section, index) => { const isOpen = expanded.includes(section.title); return <div key={section.title} className="border-t" style={{ borderColor: "#cfc6b8" }}><button type="button" onClick={() => setExpanded((current) => isOpen ? current.filter((title) => title !== section.title) : [...current, section.title])} className="flex w-full items-center justify-between py-3 text-left text-[11px] font-semibold" style={{ color: "#4b5b52" }}><span className="flex items-center gap-2"><span className="font-mono text-[9px]" style={{ color: "#b2573b" }}>0{index + 1}</span>{section.title}</span>{isOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</button>{isOpen && <p className="pb-3 pl-7 text-[11px] leading-5" style={{ color: "#70776f" }}>{section.body}</p>}</div>; })}</div>
              <div className="mt-6 flex gap-2"><button type="button" onClick={() => setShowCollections(true)} className="flex flex-1 items-center justify-center gap-2 border px-3 py-2.5 text-[10px] font-semibold uppercase tracking-[.09em] transition-colors" style={{ borderColor: savedIn.length ? "#4c8176" : "#b2573b", background: savedIn.length ? "#dcece4" : "#b2573b", color: savedIn.length ? "#2f6e63" : "#f7f3eb" }}>{savedIn.length ? <BookmarkCheck size={13} /> : <Bookmark size={13} />}{savedIn.length ? `Saved in ${savedIn.length}` : "Save passage"}</button><button type="button" onClick={copyReference} aria-label="Copy skill reference" className="flex w-11 items-center justify-center border" style={{ borderColor: "#b8aa99", color: copied ? "#2f6e63" : "#777b70" }}>{copied ? <Check size={14} /> : <Copy size={14} />}</button></div>
      ...
      {showNewCollection && <div className="fixed inset-0 z-40 flex items-center justify-center bg-[#293731]/25 px-5" onClick={() => setShowNewCollection(false)}><div className="w-full max-w-[390px] border p-5 shadow-[0_16px_40px_rgba(38,50,46,.16)]" style={{ borderColor: "#b8aa99", background: "#f7f3eb" }} onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><div className="font-mono text-[9px] uppercase tracking-[.15em]" style={{ color: "#b2573b" }}>New shelf</div><h3 className="mt-2 text-[24px]" style={{ fontFamily: "Georgia, serif" }}>Name your collection.</h3></div><button type="button" aria-label="Close new collection" onClick={() => setShowNewCollection(false)} style={{ color: "#85877f" }}><X size={16} /></button></div><p className="mt-2 text-[11px] leading-5" style={{ color: "#777b70" }}>The current passage will be saved there first.</p><input autoFocus value={newCollectionName} onChange={(event) => setNewCollectionName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") createCollection(); }} placeholder="e.g. Monday handoffs" className="mt-5 h-11 w-full border bg-transparent px-3 text-[12px] outline-none focus:border-[#b2573b]" style={{ borderColor: "#b8aa99", color: "#293731" }} /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setShowNewCollection(false)} className="px-3 py-2 text-[10px] uppercase tracking-[.1em]" style={{ color: "#777b70" }}>Cancel</button><button type="button" onClick={createCollection} className="flex items-center gap-2 px-4 py-2 text-[10px] font-semibold uppercase tracking-[.1em]" style={{ background: "#b2573b", color: "#f7f3eb" }}><Plus size={13} /> Add shelf</button></div></div></div>}
    </main>
  );
}

export default ExploreF;