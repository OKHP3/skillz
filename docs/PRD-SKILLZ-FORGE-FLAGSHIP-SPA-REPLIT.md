...
The catalog must remain generated from repository content. Do not hardcode these numbers in the UI. Regenerate `forge/src/data/catalog.json` from `forge/scripts/build-catalog.js` during the app build so repository additions, renamed package identifiers, and improvements become visible automatically.
