...
> **Repository-context refresh: 2026-07-24.** Family count, family list, and
> distribution skill count below were verified against a live `okhp3-skill-cataloger --full`
> run (`.catalog-meta.json`) at the time of this edit: **11 active families, 68 distribution
> skills**. If Replit is rebuilding from this directive, re-verify these two numbers against
> the repository's current `.catalog-meta.json` rather than hardcoding what is written here —
> the catalog is expected to keep growing.
...
The repository is `OKHP3/skillz`. It contains Agent Skills in 11 active families, plus placeholder directories (`askjamie`, `glee-fully`) and project-local support skills under `.agents/skills/`. The root `AGENTS.md` is the routing index. The root `README.md` is the human catalog. The `docs/` directory contains public-surface, publishing, security, roadmap, and architecture guidance.

The repository currently includes these active families:
...
The repository includes 68 distribution skills in the current local catalog (`.catalog-meta.json`, regenerated by `okhp3-skill-cataloger --full`). This includes the new `okhp3-database-cartographer` universal skill. Some descriptions and metadata are more complete than others. The UI must represent maturity honestly. Do not imply that every skill is equally complete, tested, or production-ready.
