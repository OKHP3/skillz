...
`artifacts/forge/scripts/build-catalog.js` builds `artifacts/forge/public/data/catalog.json`, which
...
We will use **Option 2**: `findSkillFiles()` in `artifacts/forge/scripts/build-catalog.js`
...
- `artifacts/forge/scripts/build-catalog.js` (`findSkillFiles`, `readFamilyDisplayName`)
