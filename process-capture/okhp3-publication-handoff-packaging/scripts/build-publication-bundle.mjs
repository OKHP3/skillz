...
export function buildPublicationBundle(dir, opts = {}) {
  ...
  if (existsSync(reportPath)) {
    ...
    errors.push('BUNDLE-1: validation-report.yaml is missing — run okhp3-process-validation-scoring first');
  }
...
function sourceSkillFor(filename) {
  if (filename === 'pir.yaml') return 'okhp3-process-intake-and-scope';
  if (filename === 'pns.yaml' || filename === 'pns.md') return 'okhp3-process-narrative-authoring';
  if (filename === 'bpmn-beta.mmd') return 'okhp3-visual-process-modeling';
  if (filename === 'sop.md' || filename === 'work-instructions.md') return 'okhp3-sop-work-instructions';
  if (filename === 'validation-report.yaml') return 'okhp3-process-validation-scoring';
  if (filename === 'stakeholder-register.yaml') return 'okhp3-stakeholder-and-role-mapping';
  if (filename === 'raci.md' || filename === 'governance-matrix.md') return 'okhp3-raci-governance-matrix';
  if (filename === 'sipoc.md') return 'okhp3-sipoc-generation';
  if (filename === 'decision-model.yaml' || filename === 'dmn-table.md') return 'okhp3-decision-model-authoring';
  if (filename === 'gap-analysis.yaml' || filename === 'exception-catalog.yaml') return 'okhp3-process-gap-exception-analysis';
  if (filename === 'measures-register.yaml' || filename === 'controls-register.yaml') return 'okhp3-process-measures-controls';
  return 'unknown';
}
